...
<!--	<xsl:output indent="yes"/>-->
				...
				<fileDesc>
					<titleStmt>
						...
						<title type="short">
							...
							<xsl:value-of select="concat($da[3], '-', $mo, '-',         format-number(number(substring-before($da[1], '.')), '00'))"/>
						</title>
						<meeting>
							<placeName>
	...
	<xsl:template match="w:r[w:endnoteReference]">
		<note type="footnote">
			<xsl:apply-templates select="/pkg:package/pkg:part[@pkg:name = '/word/endnotes.xml']/pkg:xmlData/w:endnotes/w:endnote[@w:id = current()/w:endnoteReference/@w:id]"/>
		</note>
	</xsl:template>
